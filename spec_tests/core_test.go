...
const coreSpecTestsDirPath = "../spec/test/core"

func TestCoreSpec(t *testing.T) {
	files, err := os.ReadDir(coreSpecTestsDirPath)
	if err != nil {
	...
	for _, file := range files {
		...
		if file.Name() == "elem.wast" {
			// Seems to be broken due to wast2wasm.
			continue
		}

		wastFile := filepath.Join(coreSpecTestsDirPath, file.Name())
