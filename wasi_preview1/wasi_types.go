...
// EventFdReadwrite represents contents for FdRead and FdWrite variants.
...
const (
	SubclockFlagsSubscriptionClockAbstime uint16 = 1 << 0
)

// SubscriptionClock is the contents of a subscription for EventTypeClock.
...
// SubscriptionFdReadwrite is the contents for FdRead or FdWrite.
type SubscriptionFdReadwrite struct {
	FileDescriptor uint32
}

// Subscription represents a subscription to an event.
// Note: This struct uses a byte array to represent the C-union `u`.
type Subscription struct {
	UserData uint64
	Type     uint8
	// Padding to align the union, typical for WASI structs.
	_ [7]byte
	// Body contains the union of SubscriptionClock and SubscriptionFdReadwrite.
	// Max size is SubscriptionClock (approx 32 bytes).
	Body [32]byte
}
